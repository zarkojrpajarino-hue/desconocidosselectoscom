          ...
          <div className="flex items-center gap-2 md:gap-3 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs md:text-sm text-muted-foreground hidden sm:block">
                Gestiona KPIs, OKRs y finanzas
              </p>
            </div>
          </div>
          <div className="flex items-center gap-1 md:gap-2 flex-shrink-0">
            <SectionTourButton sectionId="metrics-hub" className="hidden sm:flex" />
            <Button
              variant="outline"
              size="sm"
      ...
      <main className="container mx-auto px-3 md:px-4 py-4 md:py-8 max-w-7xl">
        <div className="mb-4 md:mb-8">
          ...
          <p className="text-sm md:text-base text-muted-foreground">
            Selecciona el sistema de métricas
          </p>
        </div>

        <div id="metrics-sections" className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
          {metricsOptions.map((option) => {
            const Icon = option.icon;
            return (
              <Card
                key={option.path}
                className="cursor-pointer hover:shadow-lg transition-all duration-300 active:scale-95 md:hover:scale-105 border-2 hover:border-primary/50 group"
                onClick={() => navigate(option.path)}
              >
                <CardHeader className="space-y-3 md:space-y-4 p-4 md:p-6">
                  <div className={`w-12 h-12 md:w-16 md:h-16 rounded-xl md:rounded-2xl bg-gradient-to-br ${option.gradient} flex items-center justify-center transition-transform group-hover:scale-110`}>
                  ...
                  <div className="pt-2 md:pt-3 border-t border-border/50 hidden md:block">
