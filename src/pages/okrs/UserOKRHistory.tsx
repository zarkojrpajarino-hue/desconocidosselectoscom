  ...
  const fetchUserInfo = async () => {
    try {
      const { data, error } = await supabase
        .from('users')
        .select('full_name, strategic_objectives')
        .eq('id', userId)
        .single();
    ...
    try {
      ...
      const historicalOKRs = await Promise.all(
        (objectivesData || []).map(async (obj) => {
          ...
          const krsWithProgressAndUpdates = await Promise.all(
            (krs || []).map(async (kr) => {
              const progress = calculateKRProgress(kr);
              
              // Obtener actualizaciones del KR con evidencias
              ...
              // Para cada update, obtener evidencias
              const updatesWithEvidences = await Promise.all(
                (updates || []).map(async (update) => {
                  const { data: evidences } = await supabase
                    .from('okr_evidences')
                    .select('file_name, file_url, file_type')
                    .eq('okr_update_id', update.id);
              ...
              return {
                ...kr,
                progress,
                updates: updatesWithEvidences
              };
            })
          );

          // Calcular progreso ponderado del objetivo
          const totalWeight = krsWithProgressAndUpdates.reduce((sum, kr) => sum + (kr.weight || 1), 0);
          const weightedProgress = krsWithProgressAndUpdates.reduce(
            (sum, kr) => sum + (kr.progress * (kr.weight || 1)),
            0
          );
          const objectiveProgress = totalWeight > 0 ? weightedProgress / totalWeight : 0;

          const achieved_krs = krsWithProgressAndUpdates.filter(kr => kr.status === 'achieved').length;
          const on_track_krs = krsWithProgressAndUpdates.filter(kr => kr.status === 'on_track').length;
          const at_risk_krs = krsWithProgressAndUpdates.filter(kr => kr.status === 'at_risk').length;
          const behind_krs = krsWithProgressAndUpdates.filter(kr => kr.status === 'behind').length;
          ...
          return {
            ...
            key_results: krsWithProgressAndUpdates,
            achieved_krs,
            on_track_krs,
            at_risk_krs,
            behind_krs,
            total_krs: krsWithProgressAndUpdates.length
          };
        })
      );
      ...
      toast.error('Error al cargar historial de OKRs');
    } finally {
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold bg-gradient-primary bg-clip-text text-transparent">
                {userInfo?.full_name} - {userInfo?.strategic_objectives || 'Sin área'}
              </h1>
              <p className="text-sm text-muted-foreground">
                Historial completo de OKRs semanales
              </p>
            </div>
          </div>
          <Button
            ...
            <Card className="border-dashed">
              <CardContent className="flex flex-col items-center justify-center py-16">
                ...
                <p className="text-muted-foreground text-center max-w-md">
                  Este usuario aún no ha generado OKRs semanales
                </p>
              </CardContent>
            </Card>
          ) : (
                      ...
                      {weekOKR.key_results.map((kr) => (
                        <Card key={kr.id} className="bg-muted/30">
                          <CardContent className="pt-4 space-y-3">
                            ...
                            {/* Botón para expandir/colapsar actualizaciones */}
                            {kr.updates.length > 0 && (
                              <>
                                <Button
                                  variant="ghost"
                                ...
                                >
                                  ...
                                  {expandedKR === kr.id ? 'Ocultar' : 'Ver'} {kr.updates.length} {kr.updates.length === 1 ? 'actualización' : 'actualizaciones'}
                                </Button>

                                {/* Historial de actualizaciones */}
                                {expandedKR === kr.id && (
                                  <div className="space-y-3 pt-2 border-t">
                                    {kr.updates.map((update) => (
                                      <div key={update.id} className="p-3 bg-background rounded-lg border space-y-2">
                                        <div className="flex items-center justify-between text-xs text-muted-foreground">
                                          <span>{formatDateTime(update.created_at)}</span>
                                          <Badge variant="outline" className="text-xs">
                                            {update.previous_value} → {update.new_value} {kr.unit}
                                          </Badge>
                                        </div>
                                        
                                        {update.comment && (
                                          <div className="text-sm whitespace-pre-wrap">{update.comment}</div>
                                        )}
                                        ...
                                        {update.evidences.length > 0 && (
                                          <div className="space-y-1">
                                            ...
                                            <div className="flex flex-wrap gap-2">
                                              {update.evidences.map((evidence, idx) => (
                                                <a
                                                  key={idx}
                                                  href={evidence.file_url}
                                                  target="_blank"
                                                  rel="noopener noreferrer"
                                                  className="flex items-center gap-1 text-xs px-2 py-1 bg-primary/10 hover:bg-primary/20 rounded border border-primary/20 transition-colors"
                                                >
